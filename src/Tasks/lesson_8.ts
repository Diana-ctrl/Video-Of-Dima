...
export function sum(...nums: Array<any>): number {
    return nums.reduce((prev, curent) => prev + curent, 0)
}
...
export function getSum(number: number): number {
    return number.toString().split('').reduce((prev, cur) => +prev + +cur, 0)
}
...
export const isEvenIndexSumGreater = (arr: Array<number>): boolean | undefined => {
    let even = arr[0];
    let uneven = arr[1];
